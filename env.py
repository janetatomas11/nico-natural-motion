...
class NicoEnv(Env):
    def __init__(self, config_file, scene_file, joints=None):
        ...
        self.io.add_sphere(name='target', size=[1, 1, 1])
        ...
        self._high = np.array([angle if angle >= 0 else 360 + angle for angle in [self._robot.getAngleUpperLimit(joint)
                                                                                  for joint in self._joints]])
        self.observation_space = Box(low=np.array([POS_MIN, POS_MIN]), high=np.array([POS_MAX, POS_MAX]))
    ...
    def _get_state(self):
        return np.array([self.handle.get_position(), self.target.get_position()])
    ...
    def reset(self):
        ...
        self.handle.set_position(observation[0])
